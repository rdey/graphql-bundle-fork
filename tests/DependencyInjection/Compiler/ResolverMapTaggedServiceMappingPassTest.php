...
final class ResolverMapTaggedServiceMappingPassTest extends TestCase
{
    public function testProcess(): void
    {
        ...
        $container->setParameter('redeye_graphql.resolver_maps', [
            'foo' => [],
            'bar' => [],
        ]);
        ...
        $container->register('App\\GraphQl\\Resolver\\ResolverMap1')
            ->addTag('redeye_graphql.resolver_map', ['schema' => 'foo']);

        $container->register('App\\GraphQl\\Resolver\\ResolverMap2')
            ->addTag('redeye_graphql.resolver_map', ['schema' => 'foo', 'priority' => 10])
            ->addTag('redeye_graphql.resolver_map', ['schema' => 'bar', 'priority' => -10]);
        ...
        $container->register('App\\GraphQl\\Resolver\\ResolverMap4')
            ->addTag('redeye_graphql.resolver_map', ['schema' => 'bar']);
    ...
    public function testProcessThrowsIfSchemaAttributeIsNotDefinedOnTag(): void
    {
        ...
        $container->register('App\\GraphQl\\Resolver\\ResolverMap')
            ->addTag('redeye_graphql.resolver_map');

        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('The "schema" attribute on the "redeye_graphql.resolver_map" tag of the "App\\GraphQl\\Resolver\\ResolverMap" service is required.');
    ...
    public function testProcessThrowsIfTagReferencesUnknownSchema(): void
    {
        ...
        $container->register('App\\GraphQl\\Resolver\\ResolverMap')
            ->addTag('redeye_graphql.resolver_map', ['schema' => 'baz']);

        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('Service "App\\GraphQl\\Resolver\\ResolverMap" is invalid: schema "baz" specified on the tag "redeye_graphql.resolver_map" does not exist (known ones are: "foo", "bar").');
